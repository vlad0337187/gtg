...
def usage():
    ...
    text = translate("gtcli -- a command line interface to gtg\n")
    text += "\n"

    text += translate("Options:\n")
    text += spaces % ("-h, --help", translate("This help"))
    text += "\n"

    text += translate("Basic commands:\n")
    text += spaces % ("gtcli new", translate("Create a new task"))
    text += spaces % ("gtcli show <tid>",
                      translate("Display detailed information on given task id"))
    text += spaces % ("gtcli edit <tid>",
                      translate("Opens the GUI editor for the given task id"))
    text += spaces % ("gtcli delete <tid>",
                      translate("Removes task identified by tid"))
    text += spaces % ("gtcli list [all|today|<filter>|<tag>]...",
                      translate("List tasks"))
    text += spaces % ("gtcli search <expression>", translate("Search tasks"))
    text += spaces % ("gtcli count [all|today|<filter>|<tag>]...",
                      translate("Number of tasks"))
    text += spaces % ("gtcli summary [all|today|<filter>|<tag>]...",
                      translate("Report how many tasks starting/due each day"))
    text += spaces % ("gtcli overview <count of days>",
                      translate("Report which tasks are scheduled on each day, \
                        default 7 days"))
    text += spaces % ("gtcli postpone <tid> <date>",
                      translate("Updates the start date of task"))
    text += spaces % ("gtcli close <tid>",
                      translate("Sets state of task identified by tid to closed"))
    text += spaces % ("gtcli browser [hide|show]",
                      translate("Hides or shows the task browser window"))
